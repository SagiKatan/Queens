...
level_mat = [[Square(SQUARE_POS_X + j * SQUARE_WIDTH, SQUARE_POS_Y + i * SQUARE_HEIGHT, i, j) for j in range(7)] for i
             in
             range(7)]
...
def update_colors(colors_dict, level_matrix):
    for key in colors_dict:
        for item in colors_dict[key]:
            level_matrix[item[0]][item[1]].set_color(key)
