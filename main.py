...
def main():
    ...
    while running:
        ...
        count_colors(level_mat, value_matrix,
                     colors_queens)  # count for each color, how many queens there are for each color
        row_flag = check_rows(value_matrix,
                              level_mat)  # Check if there is a one or more rows with wrong placement.If there is at least 1 row -row_flag is True
        column_flag = check_columns(value_matrix,
                                    level_mat)  # Check if there is a one or more columns with wrong placement.If there is at least 1 column -column_flag is True
        frame_flag = check_frames(value_matrix,
                                  level_mat)  # Check for each queen if there is a queen in its frame.
        # if there is at least 1 queen with the wrong frame -frame_flag is True
        color_flag = check_colors(colors_queens, colors,
                                  level_mat)  # Check if there is more than 1 queen in the same color.if there is -a color flag is True
        ...
        if all_queens_flag and not error_flag:  # the game will be finished
            # when all_queens_flag is True and error_flag is False
